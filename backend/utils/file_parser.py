...
# Prefer the maintained ``pypdf`` package; fall back to legacy ``PyPDF2``.
try:  # pragma: no cover - import selection depends on environment
...
logger = logging.getLogger("resume_analyzer.file_parser")
...
def extract_text_from_pdf(file_path: str) -> Optional[str]:
    ...
    # Fallback to pypdf/PyPDF2 if pdfplumber fails
    if not text.strip():
        try:
...
def save_uploaded_file(uploaded_file, upload_dir: str) -> Optional[str]:
    ...
    try:
        ...
        # (e.g. a crafted name like "../../etc/passwd").
        ...
        logger.warning("Failed to save uploaded file", exc_info=True)
        return None
