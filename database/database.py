...
# Database file path
# Priority:
#   1. RESUME_ANALYZER_DB env var (explicit override, used for tests/deployments)
#   2. Streamlit Cloud ephemeral /tmp storage
#   3. Local project directory
if os.environ.get("RESUME_ANALYZER_DB"):
    DB_PATH = os.environ["RESUME_ANALYZER_DB"]
elif os.environ.get("STREAMLIT_SERVER_HEADLESS"):
...
def _run_migrations() -> None:
    ...
    migrations = {
        ...
        "analysis_results": {
            ...
            "analysis_type": "VARCHAR(30) DEFAULT 'Resume + Job Description'",
            "quality_score": "INTEGER DEFAULT 0",
        },
    }

    try:
        # Relax NOT NULL constraints on analysis_results (job_id / match %)
        # for pre-existing databases before applying additive column changes.
        _relax_analysis_results_nullability()
...
def _relax_analysis_results_nullability() -> None:
    """Rebuild ``analysis_results`` so ``job_id`` and ``job_match_percentage``
    allow NULLs (required for the "Resume Only" workflow).

    SQLite cannot ALTER a column to drop a NOT NULL constraint, so we rebuild
    the table using the current ORM definition and copy the existing rows over.
    This is a no-op when the columns are already nullable (fresh databases).
    """
    from sqlalchemy import inspect, text
    ...
    cols = {c["name"]: c for c in inspector.get_columns("analysis_results")}
    job_id_col = cols.get("job_id")
    match_col = cols.get("job_match_percentage")

    needs_rebuild = (
        (job_id_col is not None and job_id_col.get("nullable") is False)
        or (match_col is not None and match_col.get("nullable") is False)
    )
    if not needs_rebuild:
        return

    from database.schema import AnalysisResult

    old_columns = list(cols.keys())
    new_columns = [c.name for c in AnalysisResult.__table__.columns]
    shared = [c for c in old_columns if c in new_columns]
    shared_sql = ", ".join(shared)

    with engine.begin() as conn:
        conn.execute(text("ALTER TABLE analysis_results RENAME TO analysis_results_old"))

    # Recreate the table with the current (nullable-friendly) ORM schema.
    AnalysisResult.__table__.create(bind=engine)

    with engine.begin() as conn:
        conn.execute(text(
            f"INSERT INTO analysis_results ({shared_sql}) "
            f"SELECT {shared_sql} FROM analysis_results_old"
        ))
        conn.execute(text("DROP TABLE analysis_results_old"))
