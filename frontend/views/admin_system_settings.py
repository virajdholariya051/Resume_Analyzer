"""
Admin System Settings - resume rules, AI, file upload, email, security, backup.
"""
...
def render_admin_system_settings() -> None:
    ...
    tabs = st.tabs([
        "Resume Rules", "AI Settings", "File Upload",
        "Email Settings", "Security", "Backup",
    ])
    ...
    with tabs[2]:
        st.write("File upload limits are governed by Resume Rules and Streamlit's "
                 "`maxUploadSize` in `.streamlit/config.toml`.")
        st.code("[server]\nmaxUploadSize = 10", language="toml")

    with tabs[3]:
        with st.form("email_settings"):
            host = st.text_input("SMTP Host", s.get("smtp_host", "") or "")
            port = st.text_input("SMTP Port", s.get("smtp_port", "587") or "587")
            user = st.text_input("SMTP User", s.get("smtp_user", "") or "")
            if st.form_submit_button("💾 Save Email Settings"):
                settings.set_many({"smtp_host": host, "smtp_port": port, "smtp_user": user})
                audit.log(admin["user_id"], "Updated Email settings")
                st.success("Email settings saved.")
        st.caption("Outbound email requires connecting these SMTP details to a mailer.")
